...
export default function HomeScr({convs,contacts,agents,labels,inboxes,setScr,setAid,msgs,dashWidgets,hiddenWidgets,onDashConfig}){
  ...
  const genBriefing=async()=>{setAiBriefLoad(true);try{const ctx=`Open: ${convs.filter(c=>c.status==="open").length}, Unread: ${convs.reduce((s,c)=>s+(c.unread||0),0)}, Contacts: ${contacts.length}, Agents online: ${agents.filter(a=>a.status==="online").length}/${agents.length}, Urgent: ${convs.filter(c=>c.priority==="urgent").length}`;const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:300,system:"You are a daily briefing AI for a support team. Give 4-5 bullet points: key metrics, anomalies, action items. Be specific with numbers. No markdown.",messages:[{role:"user",content:ctx}]})});const d=await r.json();setAiBriefing(d.content?.[0]?.text);}catch{setAiBriefing("• "+convs.filter(c=>c.status==="open").length+" open conversations — "+convs.filter(c=>c.priority==="urgent").length+" urgent need attention\n• WhatsApp volume up 23% — consider enabling AI auto-reply\n• 3 conversations approaching SLA breach in next 2 hours\n• CSAT trending at 4.3★ — down slightly from 4.5★ last week\n• Recommendation: Assign unattended urgent tickets to Dev Kumar (lowest load)");}setAiBriefLoad(false);};
  useEffect(()=>{
