...
export default function BotBuilderScr(){
  ...
  const handleKbFile=async(e)=>{
    ...
    try{
      ...
      const res=await fetch("http://localhost:3001/api/upload",{method:"POST",headers:{Authorization:"Bearer "+token},body:form});
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {k.url?(
                          <div style={{fontSize:11,color:C.a,fontFamily:FM}}>
                            {k.type==="image"?<img src={`http://localhost:3001${k.url}`} alt={k.title} style={{maxHeight:80,borderRadius:6,marginTop:4,display:"block"}}/>:<span>{k.content}</span>}
                          </div>
                        ):(
