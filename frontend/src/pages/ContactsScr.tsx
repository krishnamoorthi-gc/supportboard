...
export default function ContactsScr({contacts,setContacts,convs,labels,comps,setComps,customFields,getCfVal,setCfVal}){
  ...
  const importCsv=()=>{if(!csvData)return;const COLORS=[C.a,C.g,C.p,C.y,C.r,C.cy,"#ff6b35","#e91e63"];let count=0;csvData.rows.forEach(row=>{const mapped={};Object.entries(csvMapping).forEach(([csvCol,field])=>{if(field&&field!=="skip")mapped[field]=row[csvCol]||"";});if(!mapped.name)return;const av=mapped.name.split(" ").map(w=>w[0]).join("").toUpperCase().slice(0,2);setContacts(p=>[...p,{id:"ct"+uid(),name:mapped.name,email:mapped.email||"",phone:mapped.phone||"",company:mapped.company||"",plan:mapped.plan||"Starter",av,color:COLORS[Math.floor(Math.random()*COLORS.length)],convs:0,csat:0,notes:mapped.notes||"",tags:[],userType:"User",location:mapped.location||"",timezone:mapped.timezone||"",language:mapped.language||"EN",currency:"USD",createdAt:new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"2-digit",year:"2-digit"})}]);count++;});showT(count+" contacts imported!","success");setShowImport(false);setCsvData(null);setCsvStep(1);setCsvMapping({});};
  ...
  const save=()=>{
    ...
    else{const nid="ct"+uid();setContacts(p=>[{id:nid,uid:"USR-"+Math.floor(3000+Math.random()*9000),...form,tags,convs:0,csat:null,totalSpend:"—",color:COLORS[Math.floor(Math.random()*COLORS.length)],av:form.name.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase(),createdAt:new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"2-digit",year:"2-digit"}),lastActivity:new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"2-digit",year:"2-digit"})},...p]);showT("Contact created!","success");if(api.isConnected())api.post("/contacts",{name:form.name,email:form.email,phone:form.phone,plan:form.plan,notes:form.notes,tags}).catch(()=>{});}
  ...
  const doMerge=()=>{if(!mergeA||!mergeB)return;setContacts(p=>p.filter(c=>c.id!==mergeB.id).map(c=>c.id===mergeA.id?{...c,notes:(c.notes||"")+(mergeB.notes?"\n"+mergeB.notes:""),tags:[...new Set([...(c.tags||[]),...(mergeB.tags||[])])],convs:(c.convs||0)+(mergeB.convs||0)}:c));showT("Contacts merged!","success");setShowMerge(false);setMergeA(null);setMergeB(null);};
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
    ...
    <div style={{flex:1,padding:"22px 24px",overflowY:"auto",minWidth:0}}>
      ...
      {bulkMode&&bulkSel.length>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",background:C.ad,border:`1px solid ${C.a}44`,borderRadius:10,marginBottom:10}}>
        ...
        <div style={{flex:1}}/>
        <Btn ch="Tag All" v="ghost" sm onClick={()=>{setContacts(p=>p.map(c=>bulkSel.includes(c.id)?{...c,tags:[...(c.tags||[]),"bulk-tagged"]}:c));showT(bulkSel.length+" contacts tagged","success");setBulkSel([]);setBulkMode(false);}}/>
        ...
        <Btn ch="Delete All" v="danger" sm onClick={()=>{setContacts(p=>p.filter(c=>!bulkSel.includes(c.id)));showT(bulkSel.length+" contacts deleted","success");setBulkSel([]);setBulkMode(false);}}/>
