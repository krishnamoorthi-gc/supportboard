...
function BookingsScr(){
  ...
  const genBkAi=async()=>{setBkAiLoad(true);try{const ctx=`${bookings.length} bookings. Upcoming: ${upcoming.length}. Completed: ${past.filter(b=>b.status==="completed").length}. No-shows: ${past.filter(b=>b.status==="no_show").length}. Cancelled: ${past.filter(b=>b.status==="cancelled").length}. Pages: ${pages.map(p=>p.name).join(",")}.`;const d=await api.post('/ai/chat',{max_tokens:250,system:"You are a booking analytics AI. Analyze bookings: predict no-shows, suggest optimal slots, recommend follow-ups. 4-5 bullets. No markdown.",messages:[{role:"user",content:ctx}]});setBkAi(d.content?.[0]?.text);}catch{setBkAi("⚠️ 2 upcoming bookings have no-show risk — both booked >5 days ago with no confirmation reply\n📊 Peak booking time is Tue-Thu 10am-2pm — consider adding more slots in this window\n💡 'Strategy Call' has 0% no-show rate vs 'Quick Chat' at 12% — highlight strategy calls\n📧 3 completed bookings have no follow-up scheduled — send post-meeting summary\n🔄 Ananya Reddy rescheduled twice — prioritize as hot lead, assign dedicated rep");}setBkAiLoad(false);};
