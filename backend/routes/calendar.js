...
router.post('/', auth, async (req, res) => {
  ...
  const ev = await db.prepare('SELECT * FROM calendar_events WHERE id=? AND created_by=?').get(id, req.agent.id);
...
router.patch('/:id', auth, async (req, res) => {
  ...
  await db.prepare(`UPDATE calendar_events SET ${sets} WHERE id=? AND created_by=?`).run(...Object.values(updates), req.params.id, req.agent.id);
  const updatedEvent = await db.prepare('SELECT * FROM calendar_events WHERE id=? AND created_by=?').get(req.params.id, req.agent.id);
